...
For this example I'm using the [Sheet component](https://ui.shadcn.com/docs/components/sheet) from shadcn/ui, but you can use the [Dialog](https://www.radix-ui.com/primitives/docs/components/dialog#api-reference) from Radix directly and apply your own style.
...
To achieve this, we need to control the open state ourselves to perform the router.back() after the closing animation, using the Radix API _onAnimationEndCapture_.
...
Now, the _onOpenChange_ only changes the open state, and _onAnimationEndCapture_ is the function that checks if the modal is closed, then calls router.back().
...
<video class="w-full rounded-md aspect-[16/10]" autoPlay muted loop controls>
  <source src="/intercepting-modal.webm" type="video/webm" />
